...
void Calc_Rusanov_Flux(double rho_L, double rho_R, double u_L, double u_R, double T_L, double T_R, double p_L, double p_R, double e_L, double e_R, double W_LOCAL_MAX,
                       double *mass_flux, double *momentum_flux, double *energy_flux, double *rhov_flux, double rhov_L, double rhov_R, double H_L, double H_R, int j){
    double mass_L = rho_L;
    ...
    double rhov_flux_L = rho_L * H_L;
    double rhov_flux_R = rho_R * H_R;
...
int main(){
   ...
   while (t<t_FINAL){
        ...
        for (int j = 1; j < N_CELLS; j++){
			double rho_L = p0[j-1];
    		double rho_R = p0[j];
            double rhov_L = p4[j-1] * rho_L; // phov is vapor density = rho * humidity
            double rhov_R = p4[j] * rho_R;
    		double u_L = p1[j-1];
    		double u_R = p1[j];
    		double T_L = p2[j-1];
    		double T_R = p2[j];
            ...
            double H_L = p4[j-1];
            double H_R = p4[j];
            double e_L = 0.5 * rho_L * u_L * u_L + (p_L / (GAMMA[j-1] - 1));
            double e_R = 0.5 * rho_R * u_R * u_R + (p_R / (GAMMA[j-1] - 1));
            ...
            Calc_Rusanov_Flux(rho_L, rho_R, u_L, u_R, T_L, T_R, p_L, p_R, e_L, e_R, W_LOCAL_MAX,
                              mass_flux, momentum_flux, energy_flux, rhov_flux, rhov_L, rhov_R, H_L, H_R, j);
