...
__global__ void GPU_Calc_new_properties(float *d_flux_X, float *d_flux_Y, float *d_p0, float *d_p1, float *d_p2, float *d_p3, float *d_p4, float R, float GAMMA,
	...
	if (INDEX < N_CELLS){
		if (i >= 2 && i < NX + 2 && j >= 2 && j < NY + 2){		//The ghost cells on the left and right are not include in calculation.
			...
			float rho_new = rho_old + (dt / dx) * (d_flux_X[L_interface + 0] - d_flux_X[R_interface + 0])
						+ (dt / dy) * (d_flux_Y[B_interface + 0] - d_flux_Y[T_interface + 0]);
			float MomX_new = MomX_old + (dt / dx) * (d_flux_X[L_interface + 1] - d_flux_X[R_interface + 1])
						  + (dt / dy) * (d_flux_Y[B_interface + 1] - d_flux_Y[T_interface + 1]);
			float MomY_new = MomY_old + (dt / dx) * (d_flux_X[L_interface + 2] - d_flux_X[R_interface + 2])
						  + (dt / dy) * (d_flux_Y[B_interface + 2] - d_flux_Y[T_interface + 2]);
			float E_new = E_old + (dt / dx) * (d_flux_X[L_interface + 4] - d_flux_X[R_interface + 4])
				    + (dt / dy) * (d_flux_Y[B_interface + 4] - d_flux_Y[T_interface + 4]);
			//更新密度 (p0)
	   		d_p0[INDEX] = rho_new;
   			// 更新動量並回推速度 (p1, p2)
