...
void Calc_flux_X(float rho_L, float rho_R, float u_L, float u_R, float v_L, float v_R, float P_L, float P_R, float e_L, float e_R, float a_L, float a_R,
	...
	float mass_L = rho_L_star;
	float mass_R = rho_R_star;
	float momentum_X_L = rho_L_star * u_L_star;
	float momentum_X_R = rho_R_star * u_R_star;
	float momentum_Y_L = rho_L_star * v_L_star;
	float momentum_Y_R = rho_R_star * v_R_star;
	float energy_L = e_L_star;
	float energy_R = e_R_star;
...
void Calc_flux_Y(float rho_B, float rho_T, float u_B, float u_T, float v_B, float v_T, float P_B, float P_T, float e_B, float e_T, float a_B, float a_T,
	...
	float mass_B = rho_B_star;
	float mass_T = rho_T_star;
	float momentum_X_B = rho_B_star * u_B_star;
	float momentum_X_T = rho_T_star * u_T_star;
	float momentum_Y_B = rho_B_star * v_B_star;
	float momentum_Y_T = rho_T_star * v_T_star;
	float energy_B = e_B_star;
	float energy_T = e_T_star;
