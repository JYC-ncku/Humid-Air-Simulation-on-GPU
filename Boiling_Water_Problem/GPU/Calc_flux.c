...
void Calc_HLL_X_flux(float rho_L, float rho_R, float u_L, float u_R, float v_L, float v_R, float T_L, float T_R, float P_L, float P_R, float Y_L, float Y_R,
		     ...
		     float *mass_flux_X, float *momentum_X_flux_X, float *momentum_Y_flux_X, float *energy_flux_X, float *mass_fraction_flux_X, int INDEX){
	...
	if (W_L >= 0.0){
		...
		mass_fraction_flux_X[INDEX] = mass_fraction_flux_L;
	} else if ( W_R <= 0.0){
		...
		mass_fraction_flux_X[INDEX] = mass_fraction_flux_R;
	} else {
		...
		mass_fraction_flux_X[INDEX] = (W_R * mass_fraction_flux_L - W_L * mass_fraction_flux_R + W_L * W_R * (mass_fraction_R - mass_fraction_L)) / (W_R - W_L);
	}
}
...
void Calc_HLL_Y_flux(float rho_B, float rho_T, float u_B, float u_T, float v_B, float v_T, float T_B, float T_T, float P_B, float P_T, float Y_B, float Y_T,
		     ...
		     float *mass_flux_Y, float *momentum_X_flux_Y, float *momentum_Y_flux_Y, float *energy_flux_Y, float *mass_fraction_flux_Y, int INDEX){
	...
	if (W_B >= 0.0){
		...
		mass_fraction_flux_Y[INDEX] = mass_fraction_flux_B;
	} else if ( W_T <= 0.0){
		...
		mass_fraction_flux_Y[INDEX] = mass_fraction_flux_T;
	} else {
		...
		mass_fraction_flux_Y[INDEX] = (W_T * mass_fraction_flux_B - W_B * mass_fraction_flux_T + W_B * W_T * (mass_fraction_T - mass_fraction_B)) / (W_T - W_B);
	}
}
