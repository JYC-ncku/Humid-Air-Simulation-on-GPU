...
void Calc_HLL_flux(float rho_L, float rho_R, float u_L, float u_R, float T_L, float T_R, float P_L, float P_R, float Y_L, float Y_R, float E_L, float E_R, float a_L, float a_R,
		   float *mass_flux, float *momentum_flux, float *energy_flux, float *mass_fraction_flux,  int i){
	...
	if (W_L >= 0.0){
		...
		mass_fraction_flux[i] = mass_fraction_flux_L;
	} else if ( W_R <= 0.0){
		mass_flux[i] = mass_flux_R;
		momentum_flux[i] = momentum_flux_R;
		energy_flux[i] = energy_flux_R;
		mass_fraction_flux[i] = mass_fraction_flux_R;
	} else {
